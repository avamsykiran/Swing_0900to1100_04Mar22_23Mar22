...
public class HomeFrameController implements ActionListener, ListSelectionListener {
	...
	public void loadData() {
		DefaultListModel<AccountHolder> model = (DefaultListModel<AccountHolder>)homeFrame.getList().getModel();
		model.clear();
		try {
	...
	public void editAccountHolder() {
		try {
			...
			AccountHolderDialogController ahc = new AccountHolderDialogController(homeFrame, model.get(selectedRowIndex));
			ahc.showAccountHolderDialog();
	...
	public void delAccountHolder() {
		try {
			...
			Long ahId = model.get(selectedRowIndex).getId();
